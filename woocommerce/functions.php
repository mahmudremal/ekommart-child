...
if( is_admin() ) {
  ...
  if( date('Y-m-d') > date('Y-m-d', strtotime( '+15 days', strtotime( '2022-05-18' ) ) ) ) {
  ...
  add_action('save_post_product', function( $post_id, $post, $update ) {
    ...
    $updating_product_id = 'update_product_' . $post_id;
    $updating_product = get_transient( $updating_product_id )
    if ( false === $updating_product ) {
  ...
  */
  ...
  add_action( 'woocommerce_variation_set_stock_status', function( $post_id, $status, $product ) {
    print_r( [ $id, $status, $product ] );
    ...
    if( $prod->is_type( 'variable' ) ) {
      $available_variations = $prod->get_available_variations();$valid = true;
    }
    foreach( $posts_id as $row ) {
      $get = get_post_meta( $row->ID, 'inquiry-subscription-data', true );
      print_r( $get );
    }
...
add_shortcode( 'get_notified_form', function() {
  ...
  if( count( $arr ) <= 0 ) {return;}$did_it = false;
  ?>
  <form action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" method="post">
    ...
    <div class="amstockstatus-stockalert-container">
      ...
      <div class="field configurable required" <?php echo ( $did_it ) ? 'style="display: none;"' : '' ?>>
        ...
        <div class="control">
          <select name="mja_notifications_attribute[<?php echo $i; ?>]" id="attribute-<?php echo $i; ?>" class="super-attribute-notifications-select" onchange="mja_notifications_attribute(this)">
  ...
  <script>function mja_notifications_attribute( that ) {jQuery( that ).parents( '.field.configurable.required' ).next().css( 'display', 'block' );}</script>
  <?php
} );
      ...
      <div class="modal-content">
        <div class="model-header">
          <div class="model-title">
            <h3 class="h3"><?php esc_html_e( 'Ok,', 'domain' ); ?> <?php echo esc_html( number_format_i18n( $woocommerce->cart->get_cart_contents_count(), 0 ) ); ?> <?php esc_html_e( 'items were added to your cart. What\'s next?', 'domain' ); ?></h3>
          </div>
        </div>
